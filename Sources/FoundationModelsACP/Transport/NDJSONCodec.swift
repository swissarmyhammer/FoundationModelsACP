...
///
/// Stdio, in-memory, and subprocess transports all satisfy this: an async
...
/// One decoded ndJSON line: either a parsed message, or a line that survived
/// framing but failed to parse as JSON.
///
/// Distinguishing the two lets the connection answer a malformed frame with a
/// clean JSON-RPC parse error instead of either crashing the read loop or
/// silently dropping wire-level garbage — a blank or whitespace-only line is
/// neither case and produces no frame at all (see `NDJSONCodec.decode`).
public enum NDJSONFrame: Sendable {
    /// A line that decoded to a JSON value.
    case message(JSONValue)

    /// A non-blank line that failed to decode as JSON, carrying a bounded
...
public enum NDJSONCodec {
    /// Decodes one framed line into a frame.
    ///
    /// - Parameters:
    ///   - line: The line payload, without its trailing newline.
    ///   - logger: Receives a diagnostic when the line fails to parse.
    /// - Returns: The decoded frame, or `nil` when the line was blank.
    public static func decode(line: Data, logger: ACPLogger) -> NDJSONFrame? {
        ...
        do {
            ...
            return .message(try JSONDecoder().decode(JSONValue.self, from: payload))
        } catch {
            let preview = String(decoding: payload.prefix(256), as: UTF8.self)
            logger.log("NDJSONCodec: malformed line \(preview.debugDescription): \(error)")
            return .malformed(preview)
        }
    }
    ...
    /// Decodes a byte stream into a stream of framed lines.
    ///
    /// - Parameters:
    ///   - bytes: Incoming byte chunks, e.g. an `ACPTransport`'s `bytes`.
    ///   - logger: Receives a diagnostic for each malformed line.
    /// - Returns: The decoded frames, finishing when `bytes` finishes.
    public static func frames(
        from bytes: AsyncThrowingStream<Data, any Error>,
        logger: ACPLogger
    ) -> AsyncThrowingStream<NDJSONFrame, any Error> {
                ...
                do {
                    for try await chunk in bytes {
                        for line in framer.append(chunk) {
                            if let frame = decode(line: line, logger: logger) {
                    ...
                    if let tail = framer.finish(),
                        let frame = decode(line: tail, logger: logger)
                    {
                        continuation.yield(frame)
                    }
                    continuation.finish()

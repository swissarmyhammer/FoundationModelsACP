...
enum Emitter {
    ...
    /// The public conformance list every generated struct and enum declares.
    ...
    /// The raw type backing every `Tag` enum keyed by a wire string —
    /// tagged-union discriminators, `anyOf` discriminators, and value-union
    /// discriminators are all wire strings, never another JSON scalar kind.
    private static let unionTagRawType = "String"

    /// The synthesized fallback case every union family adds so an
    /// unrecognized discriminator decodes without error and re-encodes
    ...
    /// The two-line opening of every hand-rolled `init(from:)`: the
    /// signature followed by the keyed-container declaration one level
    /// deeper, shared by struct members, tagged unions, discriminated
    /// unions, value unions, and objects carrying a union.
    ///
    /// - Parameter baseIndent: The indentation of the `init(from:)`
    ///   signature line; the container declaration indents one level deeper.
    /// - Returns: The two lines, relative to `baseIndent`.
    private static func decoderInitSequence(baseIndent: String = indentUnit) -> [String] {
        [
            baseIndent + decoderInitOpening,
            baseIndent + indentUnit + decoderContainerDeclaration,
        ]
    }

    /// The four-line opening of every hand-rolled `encode(to:)`: its two doc
    /// lines, the signature, and the keyed-container declaration one level
    /// deeper, shared by struct members, tagged unions, discriminated
    /// unions, value unions, and objects carrying a union.
    ///
    /// - Parameter baseIndent: The indentation of the doc lines and the
    ///   `encode(to:)` signature; the container declaration indents one
    ///   level deeper.
    /// - Returns: The four lines, relative to `baseIndent`.
    private static func encoderInitSequence(baseIndent: String = indentUnit) -> [String] {
        [
            baseIndent + encoderParameterDoc,
            baseIndent + encoderThrowsDoc,
            baseIndent + encodeMethodOpening,
            baseIndent + indentUnit + encoderContainerDeclaration,
        ]
    }
    ...
    ///
    /// Backslashes, quotes, and the control characters that cannot appear
    /// literally inside a `"..."` string (newline, tab, carriage return) are
    /// escaped so no schema value can break out of the generated source — or
    /// simply produce a source file that fails to compile, since an
    /// unescaped newline inside a string literal is a syntax error.
    /// `swiftCaseName` already rejects such values at the generator
    /// boundary; the emitter stays safe on its own terms. Backslash is
    /// escaped first so the sequences this function introduces are never
    ...
    private static func encodeMethod(model: StructModel) -> [String] {
        ...
        lines.append(contentsOf: [indentUnit + "///"] + encoderInitSequence())
        for property in model.properties {
    ...
    private static func unionCodingKeysDeclaration(cases: [String], baseIndent: String) -> [String] {
        [baseIndent + codingKeysEnumDeclaration]
    ...
    static func taggedUnionDeclaration(model: TaggedUnionModel) -> String {
        ...
        lines.append(unknownUnionCaseDeclaration)
        lines.append("")
    ...
    private static func valueUnionEncoder(model: ObjectValueUnionModel) -> [String] {
        var lines = [
            ...
            indent2 + "///",
        ] + encoderInitSequence(baseIndent: indent2) + [
            indent3 + "switch self {",
        ]
        for unionCase in model.cases {
            switch unionCase.selector {

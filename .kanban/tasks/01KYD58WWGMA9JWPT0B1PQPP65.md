...
- actor: claude-code
  id: 01kynhdts4xhzpnfhd2vz2cnc1
  text: |-
    Pulled back from review into doing to address the 2026-07-28 18:19 review's 2 items.

    1. DECLINED (not touched): rename nextRequestID back to nextRequestId. This is the 4th round citing the same casing rule to flip this identifier, after 2026-07-27 21:01, 2026-07-27 21:19, and 2026-07-28 16:36 already litigated it and settled on nextRequestID as the rule-mandated form (interior/trailing ID acronym, leading word is "next" not "Id"/"ID"). Left nextRequestID exactly as-is; description checklist item marked with a note pointing back at the prior resolution comments per the task instructions rather than touching the identifier again.

    2. FIXED: dispatchSingle's cognitive complexity. Read the full current implementation via get symbol before editing. Extracted a private nested enum MessageKind (cases .request, .notification, .cancelRequest, .response, .unclassifiable, .malformed) plus a pure private static func classify(fields:) -> MessageKind that reproduces the original branch-for-branch classification logic in the same order. Also split the jsonrpc-version-mismatch branch out into its own private func rejectWrongVersion(fields:batchToken:) async. dispatchSingle now reduces to: guard object, guard version (delegating on failure), then a flat switch over classify(fields:) with one line per case. No behavior change intended or observed.

    Verification (all fresh this round): swift build --build-tests clean (0/0). Full swift test: 198/16 + 95/12 = 293/28, matching baseline exactly. mcp__sah__diagnostics check working: 0/0. swift test --filter PromptLifecycleTests --maximum-repetitions 30: 8/8 green across all 30 reps, including the 500-iteration internal ordering-race check in promptResponseArrivesBeforeTheFirstStateUpdate. swift test --filter 'ConnectionTests|SubprocessReapTests|SessionLifecycleTests': 46/46 green, covering cancelRequestNotificationCancelsRunningHandlerAndAnswersRequestCancelled, closingTransportReapsSpawnedChild, closingConnectionReapsChildAgent, concurrentBidirectionalRequestsCorrelateToCallers.

    Mutation testing: broke the .cancelRequest branch in classify (added an unreachable extra guard condition so $/cancel_request would misclassify as an ordinary notification instead of routing to handleCancelRequest). Rebuilt, reran the ConnectionTests/SubprocessReapTests/SessionLifecycleTests filter: caught hard, cancelRequestNotificationCancelsRunningHandlerAndAnswersRequestCancelled failed with a 60-second timeout waiting for cancellation that never happened. Reverted; reconfirmed full suite green (293/28) and both critical filters green again.

    Adversarial double-check (via really-done, double-check agent): verdict PASS, no findings. It independently traced all six MessageKind branches and rejectWrongVersion's three sub-cases against the original inline logic, confirmed the .response case's fields -> responseFields rename didn't change which dictionary reaches resolve(id:fields:), confirmed nextRequestID is untouched by the diff, confirmed classify/MessageKind/rejectWrongVersion are all private with no other call sites in the repo, and ran its own fresh diagnostics check on Connection.swift.
...
## Review Findings (2026-07-28 18:19)

Scope reviewed: `review sha HEAD~1..HEAD` (commit `1890d7f`, the checkpoint that fixed the sole previously-open finding by having `cancelOutbound` call the extracted `outboundEnvelope(id:method:params:)` helper instead of manually building its notification envelope inline).

- [x] `Sources/FoundationModelsACP/Connection/Connection.swift:190` — Variable name uses capital acronym `ID` (`nextRequestID`) instead of camelCase `Id` (`nextRequestId`), inconsistent with Swift naming conventions and the pattern established elsewhere in the file. Rename `nextRequestID` to `nextRequestId` (line 190 definition, line 196 increment, line 195 use). **DECLINED 2026-07-28 — known documented validator false-positive, not actioned, `nextRequestID` left exactly as-is.** This is (at minimum) the 4th round citing the identical casing rule to flip this same identifier, after 2026-07-27 21:01, 2026-07-27 21:19, and 2026-07-28 16:36 above already litigated it and settled on `nextRequestID` as the rule-mandated form: the project's casing rule requires the up-cased form here (`Id`/`ID` in interior/trailing, non-leading position resolves to `ID`, since the leading word is `next`, not `Id`/`ID`), and the rule text itself names flip-flopping across rounds as the validator error, not a signal to act on. See the 2026-07-28 16:21 round's note above for the fullest citation of the rule text. No rename applied; `nextRequestID` remains as settled in the 2026-07-28 21:18 resolution.
- [x] `Sources/FoundationModelsACP/Connection/Connection.swift:355` — dispatchSingle has high cognitive complexity: multiple conditional branches (5+ main branches), complex boolean logic (e.g., `fields[Self.resultKey] != nil || fields[Self.errorKey] != nil` on line 388), and multiple intertwined decision paths handling different message types make the function hard to reason about. Extract message type classification into a separate helper function that returns an enum, then pattern match on that result. This reduces dispatchSingle to a cleaner dispatch table. Consider extracting each message-type handler (request, notification, response, cancel_request) into its own private method. **FIXED 2026-07-28**: read `dispatchSingle`'s full implementation before editing (via `get symbol`) rather than trusting the cited line number, since this file's line numbers have shifted across every prior round. Extracted a private nested `enum MessageKind` with cases `.request(id:method:params:)`, `.notification(method:params:)`, `.cancelRequest(params:)`, `.response(id:fields:)`, `.unclassifiable(id:)`, and `.malformed`, plus `private static func classify(fields: [String: JSONValue]) -> MessageKind` — a pure function (touches no actor state) that reproduces the original's classification logic branch-for-branch, in the same order (method-string check first, then result/error-shaped check, then id-bearing-but-unclassifiable, then malformed). Additionally extracted the jsonrpc-version-mismatch branch (previously inline, itself doing its own id/result/error triage) into `private func rejectWrongVersion(fields:batchToken:) async`. `dispatchSingle` now reduces to: guard the message is an object, guard the jsonrpc version (delegating to `rejectWrongVersion` on failure), then a flat `switch Self.classify(fields: fields)` with exactly one line per case delegating to the pre-existing handler methods (`dispatchRequest`, `notificationHandler?`, `handleCancelRequest`, `resolve`, `respond`, `log`) — each message-type handler was already its own private method (`handleCancelRequest`, `dispatchRequest`) or single-call-site logic (notification handler invocation, `resolve`), so the extraction request's \"each into its own private method\" was already satisfied by the existing structure; what was missing and is now fixed is the classification-from-handling separation. No behavior change: this is a pure refactor.

  Verified: `swift build --build-tests` clean (0 errors/0 warnings). Full `swift test`: 198 tests/16 suites + 95 tests/12 suites = 293/28, all green, matching baseline exactly. `mcp__sah__diagnostics check working`: 0 errors/0 warnings. `swift test --filter PromptLifecycleTests --maximum-repetitions 30`: 8/8 tests green across all 30 repetitions, including the internal 500-iteration ordering-race check inside `promptResponseArrivesBeforeTheFirstStateUpdate` — the response-before-first-`state_update` guarantee holds. `swift test --filter 'ConnectionTests|SubprocessReapTests|SessionLifecycleTests'`: 46/46 green, explicitly covering `cancelRequestNotificationCancelsRunningHandlerAndAnswersRequestCancelled`, `closingTransportReapsSpawnedChild`, `closingConnectionReapsChildAgent`, `concurrentBidirectionalRequestsCorrelateToCallers`.

  **Mutation testing**: temporarily broke the `.cancelRequest` classification branch in `classify` by adding an unreachable extra guard condition (`if id == nil, false, method == Self.cancelRequestMethod`), so an incoming `$/cancel_request` notification would fall through and be misclassified as an ordinary `.notification` instead of routed to `handleCancelRequest`. Rebuilt and reran `swift test --filter 'ConnectionTests|SubprocessReapTests|SessionLifecycleTests'`: caught hard — `cancelRequestNotificationCancelsRunningHandlerAndAnswersRequestCancelled` failed with a 60-second timeout waiting for a cancellation that never happened (`try await reader.next()` returned `nil`), plus 3 total issues in the run. This confirms the test suite genuinely exercises the extracted classification path for the cancellation route this task has been protecting through 7 review rounds, not just superficially. Reverted the mutation; rebuilt clean and reconfirmed the full suite green (293/28, 0/0 diagnostics) plus the two critical filters green again.

...
## Review Findings (2026-07-14 21:44)

Scope: HEAD~1..HEAD (commit c4e44ce), same per-file scoping (Models.generated.swift exceeds the engine batch cap; its delta this iteration is the mechanical RequestId→RequestID reference rename).

- [x] `Sources/acp-generate/main.swift:39` — Path traversal via unvalidated schemaPath: user-supplied command-line argument flows directly into file read without validation for absolute paths or '..' segments, allowing arbitrary file access. Add path validation for schemaPath before line 39, matching the outputPath validation: `guard !schemaPath.hasPrefix("/"), !schemaPath.split(separator: "/").contains("..") else { die(...) }`.
- [x] `Sources/acp-generate/main.swift:51` — The string prefix "acp-generate: " is repeated across four error messages (lines 51, 54, 56, 58) and should be extracted as a named constant so it changes in one place. Extract a constant at the top of the script: `let toolPrefix = "acp-generate: "` and interpolate it into each error message, e.g., `die(message: "\(toolPrefix)output-dir must be...")`.
- [x] `Tests/ACPGenerateTests/ForgivingDecodingTests.swift:84` — Parameter documentation uses internal name `json` instead of the external label `from` used at the call site; documented parameter names must match the signature. Change `///   - json: The JSON document text.` to `///   - from: The JSON document text.`.
- [x] `Tests/ACPGenerateTests/GeneratorCoreTests.swift:86` — Parameter documentation uses internal name `name` instead of the external label `named` used at the call site; documented parameter names must match the signature. Change `/// - Parameter name:` to `/// - Parameter named:`.
...
- actor: wballard
  id: 01kxjv1m740g5dtnfhv2bqqkz6
  text: |-
    Review pass 2: 15 findings, all fixed:

    - CodingKeys wire-name escaping (2 findings, same site): codingKeys() now routes property.wireName through Emitter.stringLiteral — the helper moved to the top of Emitter (it now serves structs, string enums, and tagged unions alike; doc updated to "wire name, value, or tag"). RED-first test quoteBearingWireNameEscapesInCodingKeys added to GeneratorCoreTests (constructs a StructModel with a quote/backslash wire name, watched fail, then pass). Every schema-derived string interpolation in Emitter.swift now goes through stringLiteral — no raw interpolation sites remain.
    - Doc-comment style (13 findings): every multi-line doc summary in Emitter.swift, TaggedUnionTests.swift, and UnknownFallbackTests.swift restructured to a period-terminated first sentence + blank /// + elaboration, including same-cause comments the engine did not cite (structDeclaration, encodeMethod) per fix-at-the-root.
...
## Review Findings (2026-07-15 06:52)

- [x] `Sources/ACPGenerateCore/Emitter.swift:35` — First line of doc comment doesn't end with period; the summary spans multiple lines without a period-terminated sentence on line 1. End the first line with a period: `/// Renders a placeholder typealias seam for a definition deferred to a later stage.` Then insert a blank `///` line before continuing with elaboration.
- [x] `Sources/ACPGenerateCore/Emitter.swift:82` — First line of doc comment doesn't end with period; the summary spans multiple lines without a period-terminated sentence on line 1. End the first line with a period: `/// Renders the public memberwise initializer, defaulting optional parameters appropriately.` Then insert a blank `///` line before continuing with elaboration.
- [x] `Sources/ACPGenerateCore/Emitter.swift:100` — property.wireName is interpolated into a CodingKey string literal without escaping, allowing a wire property name containing quotes or backslashes to break out of the string literal and generate invalid Swift syntax. Use stringLiteral(property.wireName) to escape the wire name: `lines.append("        case \(property.swiftName) = \(stringLiteral(property.wireName))")`.
- [x] `Sources/ACPGenerateCore/Emitter.swift:115` — First line of doc comment doesn't end with period; the summary spans multiple lines without a period-terminated sentence on line 1. End the first line with a period: `/// Renders init(from:), decoding invariant and annotated fields appropriately.` Then insert a blank `///` line before continuing with elaboration.
- [x] `Sources/ACPGenerateCore/Emitter.swift:151` — Schema-derived property wire names are embedded in generated CodingKeys enum string literals without escaping, violating the same invariant applied to enum and union wire values elsewhere. If wireName contains quotes or backslashes, the generated code is syntactically invalid. Apply `stringLiteral()` to `property.wireName` on line 151: change to `"case \(property.swiftName) = \(stringLiteral(property.wireName))\"`.
- [x] `Sources/ACPGenerateCore/Emitter.swift:196` — First line of doc comment doesn't end with period; the summary spans multiple lines without a period-terminated sentence on line 1. End the first line with a period: `/// Renders a string enum with hand-rolled Codable, mapping snake_case wire strings to camelCase cases.` Then insert a blank `///` line before continuing with elaboration.
- [x] `Sources/ACPGenerateCore/Emitter.swift:242` — First line of doc comment doesn't end with period; the summary spans multiple lines without a period-terminated sentence on line 1. End the first line with a period: `/// Renders schema-derived text as a double-quoted Swift string literal that escapes backslashes and quotes safely.` Then insert a blank `///` line before continuing with elaboration.
- [x] `Sources/ACPGenerateCore/Emitter.swift:289` — First line of doc comment doesn't end with period; the summary spans multiple lines without a period-terminated sentence on line 1. End the first line with a period: `/// Renders a tagged union as an enum with associated payload values and discriminator-keyed Codable.` Then insert a blank `///` line before continuing with elaboration.
- [x] `Tests/ACPGenerateTests/TaggedUnionTests.swift:7` — First line of doc comment doesn't end with period; the summary spans multiple lines without a period-terminated sentence on line 1. End the first line with a period: `/// Reduces JSON bytes to a canonical sorted-keys form for byte-equivalent comparisons.` Then insert a blank `///` line before continuing with elaboration.
- [x] `Tests/ACPGenerateTests/TaggedUnionTests.swift:26` — First line of doc comment doesn't end with period; the summary spans multiple lines without a period-terminated sentence on line 1. End the first line with a period: `/// Decodes a fixture, re-encodes it, and asserts byte-equivalence and round-trip equality.` Then insert a blank `///` line before continuing with elaboration.
- [x] `Tests/ACPGenerateTests/TaggedUnionTests.swift:45` — First line of doc comment doesn't end with period; the summary spans multiple lines without a period-terminated sentence on line 1. End the first line with a period: `/// Round-trips every discriminator variant of tagged unions against wire fixtures.` Then insert a blank `///` line before continuing with elaboration.
- [x] `Tests/ACPGenerateTests/TaggedUnionTests.swift:197` — First line of doc comment doesn't end with period; the summary spans multiple lines without a period-terminated sentence on line 1. End the first line with a period: `/// Tests tagged-union generation from oneOf definitions with discriminator-carrying object variants.` Then insert a blank `///` line before continuing with elaboration.
- [x] `Tests/ACPGenerateTests/UnknownFallbackTests.swift:7` — First line of doc comment ends with colon instead of period; the summary spans multiple lines without a period-terminated sentence on line 1. End the first line with a period: `/// Tests string-enum generation and the unknown(String) fallback for forward compatibility.` Then insert a blank `///` line before continuing with elaboration.
- [x] `Tests/ACPGenerateTests/UnknownFallbackTests.swift:115` — First line of doc comment doesn't end with period; the summary spans multiple lines without a period-terminated sentence on line 1. End the first line with a period: `/// Runtime acceptance of the unknown(String) fallback for forward compatibility.` Then insert a blank `///` line before continuing with elaboration.
- [x] `Tests/ACPGenerateTests/UnknownFallbackTests.swift:178` — First line of doc comment doesn't end with period; the summary spans multiple lines without a period-terminated sentence on line 1. End the first line with a period: `/// Asserts every (wire string, Swift case) pair of a string enum decodes and re-encodes correctly.` Then insert a blank `///` line before continuing with elaboration.